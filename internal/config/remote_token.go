...
const (
	...
	DefaultRemoteTokenKeyringAccount = "gitcrawl"
)
...
func ResolveRemoteTokenWithKeyring(cfg Config) (TokenResolution, error) {
	cfg.Remote.Normalize()
	source := strings.TrimSpace(cfg.Remote.Auth.TokenSource)
	switch source {
	case "", "env":
		if token := ResolveRemoteToken(cfg); token.Value != "" {
			return token, nil
		}
